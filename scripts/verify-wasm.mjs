#!/usr/bin/env node
// CI byte-identical reproducibility check for c.wasm.
...
const committed = await readFile(path.join(repoRoot, "c.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed c.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-c-verify-"));
...
console.log(`rebuilt c.wasm sha256: ${rebuiltHash}`);
