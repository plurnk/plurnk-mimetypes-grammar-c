#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-c.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-c-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-c.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as c.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "c.wasm"));
const bytes = (await readFile(path.join(repoRoot, "c.wasm"))).length;
console.log(`c.wasm: ${bytes} bytes (built from ${pin})`);
